...
This assignment requires you to develop a simple 3D game program that implements a simple adventure game in which
...
The game scene includes a ground plane. When the game starts the only 3D object that appears in the scene is the ground plane.
...
Checking for object collisions will be an O(N2) complexity operation (for N = # of 3D objects).
...
Note: this is an O(N2) computation.
